<template>
  <div class="max-w-screen-2xl m-auto py-6 px-6">
    <h2 class="text-4xl font-black mb-4 text-center">Top rated movies</h2>
...
export default {
  ...
  async created() {
    ...
    this.loading = true
    try { this.movies = (await getTopRatedMovies(1)).results || [] } finally { this.loading = false }
  },
  methods: {
    async loadMore() {
      const { getTopRatedMovies } = useTmdb()
      ...
      try {
        ...
        const res = await getTopRatedMovies(next)
        this.page = next
