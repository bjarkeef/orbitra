<template>
  <NuxtLink
    :to="href"
    class="group block rounded-xl overflow-hidden bg-slate-900 ring-1 ring-slate-800/80 hover:ring-indigo-500/50 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-indigo-950/30"
  >
    <div class="relative aspect-[2/3] bg-slate-950">
      <img
        v-if="src"
        :src="src"
        :alt="label"
...
const href = computed(() => {
  if (kind.value === 'tv') return `/tv/${props.item.id}`
  if (kind.value === 'person') return `/actor/${props.item.id}`
  return `/m/${props.item.id}`
})
