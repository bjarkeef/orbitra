<template>
  <section class="max-w-screen-2xl mx-auto px-4 sm:px-6">
    ...
    <div v-if="loading" class="flex gap-4 overflow-hidden" aria-busy="true">
      <div
        v-for="n in 6"
        :key="n"
        class="shrink-0 w-36 sm:w-44 rounded-xl overflow-hidden bg-slate-900 ring-1 ring-slate-800/80 animate-pulse"
      >
        <div class="relative aspect-poster bg-slate-800/90">
          <div class="absolute top-2 right-2 h-6 w-11 rounded-md bg-slate-900/70 ring-1 ring-slate-600/40" />
        </div>
      </div>
    </div>
