<template>
  <div>
    <SkeletonDetailPage v-if="pending && !person" :show-gallery="true" :show-extra-block="true" />
      ...
      <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 -mt-4 relative z-10 space-y-8">
        ...
        <section class="grid gap-6 lg:grid-cols-3">
          ...
          <div class="section-card flex flex-col gap-4">
            ...
            <div
              v-if="extrasPending && !personGalleryImages?.profiles?.length"
              class="h-20 rounded-lg bg-slate-800 animate-pulse"
            />
          </div>
        </section>

        <!-- Single photo gallery (modal lightbox) — avoid duplicating sidebar thumbs -->
        <UIMediaGallery
          v-if="personGalleryImages?.profiles?.length"
          title="Photos"
          subtitle="Profile images from TMDB · click to enlarge"
          :images="personGalleryImages"
          prefer="profiles"
          aspect="poster"
          :max="24"
        />
...
const {
  ...
  () => `person-detail-${id.value}`,
  () => getPersonDetailed(id.value),
  { watch: [id] },
)

const person = computed(() => personDetailed.value || null)
const extrasPending = computed(() => pending.value)

const bundle = computed(() => {
  const credits = personDetailed.value?.combined_credits
  if (!credits) return null
  return aggregatePersonCredits(credits, {
...
const externalLinks = computed(() =>
  personExternalLinks(
    personDetailed.value?.external_ids,
    person.value?.homepage,
  ),
)

const personGalleryImages = computed(() => personDetailed.value?.images || null)
...
const backdropImgPath = computed(() => {
  ...
  const posterFallback = bundle.value?.titles?.find(t => t.poster_path)?.poster_path
  return backdropStyle(
    fromCredits?.backdrop_path,
    posterFallback || person.value?.profile_path,
  )
})

const pageTitle = computed(() =>
  person.value?.name ? `${person.value.name} · Orbitra` : 'Person · Orbitra',
)

const pageDescription = computed(() => {
  const bio = person.value?.biography?.trim()
  if (bio) return bio.slice(0, 160)
  const known = person.value?.known_for_department
  return person.value?.name
    ? `${person.value.name}${known ? ` · ${known}` : ''} — credits and co-star orbit on Orbitra.`
    : 'Person profile on Orbitra.'
})

const ogImage = computed(() =>
  imageUrl(person.value?.profile_path, 'w500') || undefined,
)
...
function titleHref(t: { media_type?: string, id: number }) {
  return t.media_type === 'tv' ? `/tv/${t.id}` : `/m/${t.id}`
}

function creditAsMediaItem(t: Record<string, any>) {
  return {
    id: t.id,
