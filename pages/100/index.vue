      ...
      <div class="relative page-shell !pb-10 !pt-10 sm:!pt-14">
        ...
        <div class="mt-6 flex flex-wrap gap-3">
          <span class="inline-flex items-center rounded-full bg-white/5 px-3 py-1 text-xs text-slate-400 ring-1 ring-white/10">
            Ranked by TMDB score · needs votes to be trustworthy
          </span>
          <span
            v-if="items.length"
      ...
      <div class="flex flex-wrap items-end justify-between gap-3 mb-6">
        <div>
          ...
          <p class="text-sm text-slate-500 mt-0.5">
            TMDB top_rated list · badges use score only when there are enough votes (avoids 10.0-from-one-vote noise)
          </p>
        </div>
      </div>
...
async function fetchPage(media: 'movie' | 'tv', pageNum: number) {
  const res
    = media === 'tv'
      ? await getTopRatedTv(pageNum)
      : await getTopRatedMovies(pageNum)
  return {
    results: (res.results || []) as TmdbMediaItem[],
