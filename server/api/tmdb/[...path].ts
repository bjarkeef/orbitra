 ...
 *
 * Catalogue policy:
 * - Upstream `include_adult` is always forced to `false`.
 * - Client-supplied `include_adult` and `api_key` query params are stripped (never trusted).
 *
 * Headers (response):
 ...
 * and always forcing `include_adult=false`.
 */
function buildUpstreamSearch(
  ...
  search.set('include_adult', 'false')
  search.set('api_key', apiKey)
  return search
}

/**
 * Query bag for cache keys: strip secrets (include_adult is always false upstream).
 */
function cacheQueryBag(query: Record<string, unknown>): Record<string, unknown> {
...
export default defineEventHandler(async (event): Promise<TmdbProxyResponse> => {
  ...
  const cacheQuery = cacheQueryBag(incomingQuery)
